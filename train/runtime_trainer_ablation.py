    ...
    if is_rank0 and config['logging'].get('use_wandb', False):
        try:
            ...
            if not hasattr(wandb, 'init'):
                log("WARNING: wandb module found but 'init' attribute missing. Checking installation...")
                import sys
                log(f"wandb module location: {wandb.__file__ if hasattr(wandb, '__file__') else 'unknown'}")
                log(f"wandb module attributes: {dir(wandb)}")
                raise ImportError("wandb.init not available - wandb may not be properly installed")
            ...
            log(f"WARNING: Failed to initialize wandb: {e}")
            log("Continuing without wandb logging...")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', type=str, default='runtime_trainer_time_token_ablation.yaml')
