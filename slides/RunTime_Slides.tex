...
\begin{frame}{Event blocks + inter-event time tokens (grammar)}
Each observed event contributes a fixed set of ``race'' tokens plus an observed pace token; \textbf{time tokens sit between events} and connect races:
\begin{itemize}
  ...
  \item outcome (observed in history): \texttt{pace\_*}
  \item inter-event cadence (connects races): \texttt{d\_next\_*}, \texttt{d\_fin\_*}
\end{itemize}
\end{frame}
...
\texttt{distance\_name\_token\_half\_marathon, pace\_\textit{?}]}
\end{center}
\normalsize
\vspace{0.3em}
...
\begin{itemize}
  ...
  \item \textbf{Swapped-token grammar}: pace now precedes cadence in the production stride to prevent leakage; the original order is no longer used.
